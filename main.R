...
raw_data <- read_excel("data/Yields_Bloomberg.xlsx", sheet = "Sheet1")

# Rename columns from Treasury0,25 to y_3, Treasury0,5 to y_6, etc.
colnames(raw_data) <- c("Date", "y_3", "y_6", "y_9", "y_12", "y_15", "y_18",
                          "y_21", "y_24", "y_30", "y_36", "y_48", "y_60",
                          "y_72", "y_84", "y_96", "y_108", "y_120",
                          "y_180", "y_240", "y_360")

ds <- raw_data

y           <- as.matrix(ds[, -1])  # Remove the date column
...
sample_A_unconstr <- function(A_curr, beta, mu, Q, sigma_prop = diag(0.01,9), accept_rate_target = 0.25) {
  ...
  e <- eigen(A_prop)
  vals <- e$values
  vals <- vals * min(0.98 / max(Mod(vals)),1)
  ...
  if (is.na(lr)) {
    lr <- -Inf  # If the log posterior is NA, set it to -Inf to reject the proposal
  }
...
# Initialize parameters as the priors
mu_curr     <- m0
A_curr      <- diag(3,3)
Q_curr      <- diag(0.01,3)
H_curr      <- rep(0.01, N)
...
# trace plots
g1 <- ggplot(df_trace, aes(x=iter, y=lambda)) + geom_line(size=1.5)
plot1 <- gg_template_save(g1, "figures/trace_lambda.png")

g2 <- ggplot(df_trace, aes(x=iter, y=a11)) + geom_line(size=1.5)
plot2 <- gg_template_save(g2, "figures/trace_A11.png")

g3 <- ggplot(df_trace, aes(x=iter, y=h11)) + geom_line(size=1.5) 
plot3 <- gg_template_save(g3, "figures/trace_H11.png")

g4 <- ggplot(df_trace, aes(x=iter, y=q11)) + geom_line(size=1.5) 
plot4 <- gg_template_save(g4, "figures/trace_Q11.png")

g5 <- ggplot(df_trace, aes(x=iter, y=mu1)) + geom_line(size=1.5)
